...
namespace DigitalWorlds.StarterPackage2D
{
    ...
    public class PlayerProjectileAttack2D : MonoBehaviour
    {
        ...
        public class AnimationParameters
        {
            ...
            public string projectileTrigger = "Projectile";
        }
        ...
        {
            [Space(20)]
            public UnityEvent onProjectileLaunched, onNoAmmo;
        ...
        [Tooltip("The input action used for the projectile attack. Set to left click by default.")]
        [SerializeField] private InputAction attackAction = new("ProjectileAttack", InputActionType.Button, "<Mouse>/leftButton");
        ...
        [Tooltip("Optional: Drag the player's animator in here for a projectile animation trigger.")]
        ...
        private bool isWaitingDelayedShot;
        ...
        public void SetAmmoCount(int count)
        {
            ...
            events.onAmmoChanged.Invoke(ammo);
        }
        ...
        private void Update()
        {
            if (attackAction.WasPressedThisFrame() && canShoot && !isOnCooldown && !isWaitingDelayedShot)
            {
                // If ammo is required, make sure we have enough for this shot
                if (requireAmmo && ammo < ammoCost)
                {
                    events.onNoAmmo.Invoke();
                    return;
                }
        ...
        private void Shoot()
        {
            ...
            if (animator != null)
            {
                animator.SetTrigger(animationParameters.projectileTrigger);
            }

            events.onProjectileLaunched.Invoke();
