...
namespace DigitalWorlds.StarterPackage2D
{
    ...
    public class PlayerMovementAdvanced : PlayerMovement2D
    {
        ...
        [Tooltip("The input action used for dashing. Set to left shift by default.")]
        [SerializeField] private InputAction dashAction = new("Dash", InputActionType.Button, "<Keyboard>/leftShift");
        ...
        protected override void Update()
        {
            ...
            // Handle dash input
            if (dashAction.WasPressedThisFrame() && canMove && dashCooldownCounter <= 0f && !isDashing)
            {
            ...
            if (animator != null)
            {
                animator.SetBool(animationParameters.isDashingBool, isDashing);
            }

            // Handle wall jump input
            if (jumpAction.WasPressedThisFrame() && isWallSliding && canMove && canWallJump)
            {
