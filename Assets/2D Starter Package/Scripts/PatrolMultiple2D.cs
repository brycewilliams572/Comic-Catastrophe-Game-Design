...
namespace DigitalWorlds.StarterPackage2D
{
    ...
    public class PatrolMultiple2D : MonoBehaviour
    {
        ...
        [Tooltip("Optional: Drag in an animator with two blend trees (idle and running). " +
            "Idle uses " + nameof(AnimationParameters.lastInputXFloat) + "/" + nameof(AnimationParameters.lastInputYFloat) + 
            ", running uses " + nameof(AnimationParameters.inputXFloat) + "/" + nameof(AnimationParameters.inputYFloat) + ". " +
            nameof(AnimationParameters.isRunningBool) + " switches between them.")]
        ...
        protected void UpdateAnimator(Vector2 inputDirection, bool isRunning)
        {
            ...
            animator.SetFloat(animationParameters.inputXFloat, inputDirection.x);
            animator.SetFloat(animationParameters.inputYFloat, inputDirection.y);

            // Update last input parameters if currently moving
            if (isRunning && inputDirection.sqrMagnitude > MAGNITUDE_THRESHOLD)
            {
                animator.SetFloat(animationParameters.lastInputXFloat, inputDirection.x);
                animator.SetFloat(animationParameters.lastInputYFloat, inputDirection.y);
            }
        }
